...
print(list("Summary of BadMentalHealth"=summary(diabetes_df$MentalHealth)))
...
HasDiabetesNo_first_ten <- list("HasDiabetesNo, BadMentalHealth, 1-10 days proprotion of data" =sum(MentalHealth_by_nozero$count[1:10])/sum(MentalHealth_by_nozero$count[1:30]))

HasDiabetesNo_middle_ten <- list("HasDiabetesNo, BadMentalHealth, 11-20 days proprotion of data" =sum(MentalHealth_by_nozero$count[11:20])/sum(MentalHealth_by_nozero$count[1:30]))

HasDiabetesNo_last_ten <- list("HasDiabetesNo, BadMentalHealth, 21-30 days proprotion of data" =sum(MentalHealth_by_nozero$count[21:30])/sum(MentalHealth_by_nozero$count[1:30]))

HasDiabetesYes_first_ten <- list("HasDiabetesYes, BadMentalHealth, 1-10 days proprotion of data"=sum(MentalHealth_by_nozero$count[31:40])/sum(MentalHealth_by_nozero$count[31:60]))

HasDiabetesYes_middle_ten <- list("HasDiabetesYes, BadMentalHealth, 11-20 days proprotion of data"=sum(MentalHealth_by_nozero$count[41:50])/sum(MentalHealth_by_nozero$count[31:60]))

HasDiabetesYes_last_ten <- list("HasDiabetesYes, BadMentalHealth, 20-30 days proprotion of data"=sum(MentalHealth_by_nozero$count[51:60])/sum(MentalHealth_by_nozero$count[31:60]))
...
confirms that the visual assessments are correct. The "No" **HasDiabetes** group is 6.4 percentage points higher in the **BadPhysicalHealth** 0-7 day range compared to the "Yes" **HasDiabetes** group. This suggests that there is a relationship between the variables. **BadPhysicalHealth* appears to be suitable for predicion modeling. 
